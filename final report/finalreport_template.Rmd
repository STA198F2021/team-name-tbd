...
## Nature of the Data
```{r distribution_pub, echo= FALSE, fig.height = 3, fig.width = 3}
spending %>%
...
```{r distribution_pri, echo= FALSE, fig.height = 3, fig.width = 3}
spending %>%
...
```{r distribution_oop, echo= FALSE, fig.height = 3, fig.width = 3}
spending %>%
...
```{r log_pub, echo= FALSE, fig.height = 3, fig.width = 3}
spending %>%
...
```{r log_pri, echo= FALSE, fig.height = 3, fig.width = 3}
spending %>%
...
```{r log_oop, echo= FALSE, fig.height = 3, fig.width = 3}
spending %>%
...
```
These graphs of the log distribution of the various spending means appear to be fairly normal in distribution, which means they meet the requirements to be used in various analyses.
...
In order to analyze spending, we must convert all mean spending reports to log scale.
...
t.test(spending_malefemale$lmean_oop~spending_malefemale$sex) %>%
  print()
```
All three of these t-tests indicate that there is not enough evidence to reject the null hypothesis that emergency department spending is the same for males and females who have public insurance, private insurance, or pay out of pocket. Add specific numbers
...
nrow(sigpairs)
```
The step-down t tests indicate 61 disease category pairs are different out of 105
## idk if the "all ages" group will mess u up because it changed the response in my age comparison
...
Here is a barplot showing the distribution of Emergency Department spending based on disease type and gender. ADD interpretation
```{r gender-diseasetype-barplot}

ggplot(data = spending_malefemale, aes(x = agg_cause, y = lmean_all, fill = sex)) +
  geom_bar(position = "dodge", stat = "identity") +
  theme(axis.text.x = element_text(angle = 45,hjust = 1)) +
  scale_y_continuous(labels = scales::comma) +
  labs(
