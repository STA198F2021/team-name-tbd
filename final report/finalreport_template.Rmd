...
```{r}
augmentaggcausefit <- augment(meanpubdiseasecatfit$fit)
```

!!this is weird, we need to discuss
```{r}
augmentaggcausefit %>%
  ggplot(aes(x = .fitted,
             y = .resid)) +
...
```{r}
ggplot(data = spending_malefemale, mapping = aes(x = agg_cause, 
                                                 y = lmean_all)) +
  geom_point(size = 0.25) +
  geom_line(data = augmentaggcausefit, mapping = aes(x = agg_cause, y = .fitted)) +
  scale_y_continuous(name="mean all", limits=c(0, 50))
